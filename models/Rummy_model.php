...
class Rummy_model extends MY_Model
{
    ...
    {
        $this->db->select('tbl_rummy_table_master.*,COUNT(tbl_users.id) AS online_members,tbl_rummy_table.no_of_players');
        ...
        if (!empty($no_of_players)) {
            $this->db->where('tbl_rummy_table.no_of_players', $no_of_players);
        }
    ...
    public function Gethistory($postData=null)
    {
        ...
        $this->db->select('tbl_rummy.*, tbl_users.name, tbl_rummy_log.game_id, tbl_table.private');
        $this->db->from('tbl_rummy');
        $this->db->join('tbl_users', 'tbl_users.id = tbl_rummy.winner_id', 'left');
        $this->db->join('tbl_rummy_log', 'tbl_rummy_log.game_id = tbl_rummy.id', 'left');
        $this->db->join('tbl_table', 'tbl_table.id = tbl_rummy.table_id', 'left');
        $this->db->group_by('tbl_rummy_log.game_id');
        ...
        if ($searchValue) {
            $this->db->group_start();
            $this->db->like('tbl_rummy.added_date', $searchValue, 'after');
            $this->db->like('tbl_users.name', $searchValue, 'after');
            $this->db->or_like('tbl_rummy.winner_id', $searchValue, 'after');
            $this->db->or_like('tbl_table.private', $searchValue, 'after');
            $this->db->or_like('tbl_rummy_log.game_id', $searchValue, 'after');
            $this->db->or_like('tbl_rummy.amount', $searchValue, 'after');
            $this->db->or_like('tbl_rummy.user_winning_amt', $searchValue, 'after');
            $this->db->or_like('tbl_rummy.admin_winning_amt', $searchValue, 'after');
            //$this->db->or_like('tbl_rummy.comission_amount', $searchValue, 'after');
        ...
        $this->db->join('tbl_table', 'tbl_table.id = tbl_rummy.table_id', 'left');
        $this->db->group_by('tbl_rummy_log.game_id');
        $this->db->order_by('tbl_rummy.added_date', 'DESC');
        // Add any necessary conditions or additional joins here
        $this->db->order_by('tbl_rummy.id', 'desc');
