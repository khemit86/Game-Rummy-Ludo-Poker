<?php
class PokerMaster_model extends MY_Model
{
    ...
    public function ViewTableMaster($id)
    {
        $Query = $this->db->where('isDeleted', False)
            ->where('id', $id)
            ->get('tbl_poker_table_master');
        return $Query->row();
    }
    
    public function AddTableMaster($data)
    {
        $this->db->insert('tbl_poker_table_master', $data);
    ...
    public function Delete($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update('tbl_poker_table_master', $data);
