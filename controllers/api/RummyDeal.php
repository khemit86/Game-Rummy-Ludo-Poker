...
class RummyDeal extends REST_Controller
{
    private $data;
    ...
    public function get_table_master_post()
    {
        ...
        if ($user[0]->rummy_deal_table_id) {
            $table_data = $this->RummyDeal_model->TableUser($user[0]->rummy_deal_table_id);
        ...
        $table_data = $this->RummyDeal_model->getTableMaster();
    ...
    public function get_table_post()
    {
        ...
        $isMaster = $this->RummyDeal_model->getTableMaster($this->data['boot_value']);
        if (empty($isMaster)) {
        ...
        $tables = $this->RummyDeal_model->getCustomizeActiveTable($table_amount);
        $seat_position = 1;
        ...
        if ($tables) {
            foreach ($tables as $value) {
                if ($value->members<$isMaster[0]->no_of_players) {
                    ...
                    $table_games = $this->RummyDeal_model->getAllGameOnTable($value->rummy_deal_table_id);
                    if (count($table_games)==0) {
                        $TableId = $value->rummy_deal_table_id;
                        $seat_position = $this->RummyDeal_model->GetSeatOnTable($TableId);
                    }
                    // }
                }
            }
        ...
        if (empty($TableId)) {
            $table_data = [
                ...
                // 'game_count' => $isMaster[0]->game_count,
                'added_date' => date('Y-m-d H:i:s'),
                'updated_date' => date('Y-m-d H:i:s')
            ];
            $TableId = $this->RummyDeal_model->CreateTable($table_data);
            ...
            //     $this->RummyDeal_model->AddTableUser($table_bot_data);
            // }
        }
        ...
        $table_user_data = [
            ...
            'total_points' => $isMaster[0]->game_count*MAX_POINTS,
            'added_date' => date('Y-m-d H:i:s'),
            'updated_date' => date('Y-m-d H:i:s')
        ];

        $this->RummyDeal_model->AddTableUser($table_user_data);

        $table_data = $this->RummyDeal_model->TableUser($TableId);
    ...
    public function get_public_table_post()
    {
        ...
        $tables = $this->RummyDeal_model->getPublicActiveTable();

        foreach ($tables as $key => $value) {
            $table_games = $this->RummyDeal_model->getAllGameOnTable($value->id);
    ...
    public function join_table_post()
    {
        ...
        if ($user[0]->rummy_deal_table_id) {
            // $data['message'] = 'You are Already On Table';
            // $data['table_id'] = $user[0]->rummy_deal_table_id;
            // $data['code'] = HTTP_NOT_ACCEPTABLE;
        ...
        $table = $this->RummyDeal_model->isTableAvail($this->data['table_id']);
        if (!$table) {
        ...
        // $table_user = $this->RummyDeal_model->TableUser($table->id);
        ...
        $table_games = $this->RummyDeal_model->getAllGameOnTable($table->id);
        if (count($table_games)!=0) {
        ...
        $table_user_data = [
            'table_id' => $table->id,
            'user_id' => $user[0]->id,
            'seat_position' => $this->RummyDeal_model->GetSeatOnTable($table->id),
            'added_date' => date('Y-m-d H:i:s'),
            'updated_date' => date('Y-m-d H:i:s')
        ];
        ...
        $table_data = $this->RummyDeal_model->TableUser($table->id);
    ...
    public function start_game_post()
    {
        ...
        $table_data = $this->RummyDeal_model->TableUserCardPosition($user[0]->rummy_deal_table_id);
        ...
        $game = $this->RummyDeal_model->getActiveGameOnTable($user[0]->rummy_deal_table_id);
        ...
        //         {
        //             $table_user_data = [
        //                 'table_id' => $value->rummy_deal_table_id,
        ...
        $last_game = $this->RummyDeal_model->getLastGameOnTable($user[0]->rummy_deal_table_id);
        if ($last_game) {
        ...
        $Cards = $this->RummyDeal_model->GetCards((count($new_table_data)*RUMMY_CARDS)+1);
        $game_data = [
        ...
        $GameId = $this->RummyDeal_model->Create($game_data);
        ...
        foreach ($new_table_data as $key => $value) {
            ...
            if ($amount!=0) {
                $this->RummyDeal_model->MinusWallet($value->user_id, $amount);
            }

            $this->RummyDeal_model->AddGameCount($value->user_id);
            ...
            $this->RummyDeal_model->AddGameLog($game_log);
        }
    ...
    public function rejoin_game_amount_post()
    {
        ...
        $table = $this->RummyDeal_model->isTableAvail($table_id);

        $game = $this->RummyDeal_model->getLastGameOnTable($table_id);
        $rejoin_log = $this->RummyDeal_model->rejoin_log($table_id, $this->data['user_id']);
    ...
    public function rejoin_game_post()
    {
        ...
        $this->RummyDeal_model->MinusWallet($this->data['user_id'], $rejoin_amount);
        ...
        $this->RummyDeal_model->Rejoin($rejoin_data);
    ...
    public function leave_table_post()
    {
        ...
        if ($game) {
            $this->RummyDeal_model->PackGame($this->data['user_id'], $game->id);
            ...
            if (count($game_users)==1) {
                ...
                $this->RummyDeal_model->updateTotalWinningAmtTable($TotalAmount, $user_winning_amt, $admin_winning_amt, $user[0]->rummy_deal_table_id, $game_users[0]->user_id);
                $this->RummyDeal_model->AddToWallet($user_winning_amt, $game_users[0]->user_id);
            }
        }

        $table_users = $this->RummyDeal_model->TableUser($user[0]->rummy_deal_table_id);

        if (count($table_users)==3) {
            foreach ($table_users as $ke => $valu) {
                if ($valu->user_type==1) {
                    $table_user_data = [
                        'table_id' => $valu->rummy_deal_table_id,
    ...
    public function pack_game_post()
    {
        ...
        if (count($game_users)==1) {
            $amount = 0;
            $this->RummyDeal_model->MinusWallet($this->data['user_id'], $amount);
            // $this->RummyDeal_model->MakeWinner($game->id, $amount, $game_users[0]->user_id);
    ...
    public function get_card_post()
    {
        ...
        // echo $chaal;
        if ($chaal==$user_id) {
            $cards = $this->RummyDeal_model->getMyCards($game->id, $user_id);
            ...
            if (empty($random_card)) {
                $this->RummyDeal_model->deleteDropCard($game->id);
                $random_card = $this->RummyDeal_model->GetRamdomGameCard($game->id);
            }
    ...
    public function drop_card_post()
    {
        ...
        $isCardAvailable = $this->RummyDeal_model->getMyCards($game->id, $user_id, $card);
        ...
        if ($isCardAvailable) {
            ...
            $this->RummyDeal_model->DropGameCards($table_user_data, $this->input->post('json'));
    ...
    public function get_drop_card_post()
    {
        ...
        $drop_card = $this->RummyDeal_model->GetAndDeleteGameDropCard($game->id);
    ...
    public function declare_post()
    {
        ...
        $cards = $this->RummyDeal_model->getMyCards($game->id, $this->data['user_id']);
        // echo count($cards);
        if (count($cards)>RUMMY_CARDS) {
        ...
        $remain_game_users = $this->RummyDeal_model->GameUser($game->id);
        ...
        $this->RummyDeal_model->Declare($data);

        // $this->RummyDeal_model->Show($game->id,$amount,$this->data['user_id']);

        // $amount = 800;
        // $this->RummyDeal_model->MinusWallet($lose_user_id,$amount);
        // $this->RummyDeal_model->MakeWinner($game->id,$amount,$user_id);
        $data['message'] = 'Success';
        // $data['winner'] = $user_id;
        $data['code'] = HTTP_OK;
        $this->response($data, HTTP_OK);
    ...
    public function declare_back_post()
    {
        ...
        $declare_log = $this->RummyDeal_model->GameLog($game->id, '', 3);
        $declare_count = count($declare_log);

        $winner_id = $declare_log[1]->user_id;
        ...
        if (count($remain_game_users)<=$declare_count) {
            ...
            if (count($table_games)>=$game_count) {
                ...
                if ($All_table_users[0]->total_points>$All_table_users[1]->total_points) {
                    $winner_user_id = $All_table_users[0]->user_id;
                    $loser_user_id = $All_table_users[1]->user_id;
                } else {
                    $winner_user_id = $All_table_users[1]->user_id;
                    $loser_user_id = $All_table_users[0]->user_id;
                }
                ...
                // // Make Winner Code
                // $TotalAmount = $this->RummyDeal_model->TotalAmountOnTable($user[0]->rummy_deal_table_id);
                // $this->RummyDeal_model->updateTotalWinningAmtTable($TotalAmount, $user[0]->rummy_deal_table_id);
                // $this->RummyDeal_model->AddToWallet($TotalAmount, $winner_user_id);
            ...
            //         $this->RummyDeal_model->updateTotalWinningAmtTable($TotalAmount, $user[0]->rummy_deal_table_id);
            //         $this->RummyDeal_model->AddToWallet($user_winning_amt, $winner_id);
            //     }
            // }
        }
    ...
    public function chat_post()
    {
        ...
        if (!empty($chat)) {
            $chat_data = [
                ...
                'chat' => $chat,
                // 'game_id' => $game->id
                'rummy_deal_table_id' => $user[0]->rummy_deal_table_id
            ];
            $chat_id = $this->RummyDeal_model->Chat($chat_data);
            $table_users = $this->RummyDeal_model->TableUser($user[0]->rummy_deal_table_id);
        ...
        $chat_list = $this->RummyDeal_model->ChatList($user[0]->rummy_deal_table_id);
        $data['message'] = 'Success';
        $data['list'] = $chat_list;
        $data['code'] = HTTP_OK;
        $this->response($data, HTTP_OK);
    ...
    public function tip_post()
    {
        ...
        if ($this->Users_model->TipAdmin($this->data['tip'], $this->data['user_id'], $user[0]->rummy_deal_table_id)) {
            $data['message'] = 'Success';
            $data['code'] = HTTP_OK;
            $this->response($data, HTTP_OK);
            exit();
    ...
    public function card_value_post()
    {
        ...
        $active_game = $this->RummyDeal_model->getActiveGameOnTable($user[0]->rummy_deal_table_id);
        $joker = "";
        if ($active_game) {
        ...
        $card_value = $this->RummyDeal_model->CardValue('', $card_1, $card_2, $card_3, $card_4, $card_5, $card_6);
        if ($card_value) {
            // echo $joker;
            // print_r($card_value);
            if ($card_value[0]==0) {
                $card_value = $this->RummyDeal_model->CardValue($joker, $card_1, $card_2, $card_3, $card_4, $card_5, $card_6);
            }
    ...
    public function status_post()
    {
        ...
        $user = $this->Users_model->UserProfile($user_id);
        $table_id = (!empty($user[0]->rummy_deal_table_id)) ? $user[0]->rummy_deal_table_id : $table_id;
        if (empty($table_id)) {
            $data['message'] = 'You Are Not On Table';
            $data['code'] = 403;
            $this->response($data, 200);
        ...
        if (!empty($table_id)) {
            // $data['start_game'] = $this->RummyDeal_model->GetStartGame($table_id);
            $data['chat_list'] = $this->RummyDeal_model->ChatList($table_id, 1);

            $table_data = $this->RummyDeal_model->TableUser($table_id);
            ...
            foreach ($table_data as $t => $u) {
                ...
                $amount = $this->RummyDeal_model->Invested($table_id, $u->seat_position)->amount;
            ...
            $active_game = $this->RummyDeal_model->getActiveGameOnTable($table_id);
            if ($active_game) {
                // $game_id = $active_game->id;
                $data['active_game_id'] = $active_game->id;
                $data['game_status'] = 1;
            ...
            $points = $this->RummyDeal_model->GetTablePoints($table_id);
            $points_arr = array();
            // if ($points) {
            //     $points_arr[0] = $points[0];
            //     $points_arr[1] = $points[1];
        ...
        $game = $this->RummyDeal_model->View($game_id);
        if (empty($game)) {
        ...
        $game_log = $this->RummyDeal_model->GameLog($game_id, 1);

        $game_users = $this->RummyDeal_model->GameAllUser($game_id);
        ...
        if ($game_log[0]->action==3) {
            ...
            // $data['game_users'] = $this->RummyDeal_model->GameAllUser($game_id);
        }
        // else
        // {
        $data['game_users'] = $this->RummyDeal_model->GameOnlyUser($game_id);
        $data['card_count'] = $this->RummyDeal_model->GetGameTableCardCount($game_id);
        // }
        $data['chaal'] = $chaal;
        ...
        // if($chaalCount>3)
        // {
        //     $this->RummyDeal_model->getMyCards($game->id,$chaal);
        // }

        if (!empty($user_id)) {
            // $data['cards'] = $this->RummyDeal_model->getMyCards($game->id,$user_id);
            $table_games = $this->RummyDeal_model->getAllGameOnTable($table_id);
            $data['round'] = count($table_games);
            if ($active_game) {
                $data['drop_card'] = $this->RummyDeal_model->GetGameDropCard($active_game->id);
            } else {
    ...
    public function share_wallet_post()
    {
        ...
        $TableUser = $this->RummyDeal_model->TableUser($user->rummy_deal_table_id);
        if (count($TableUser)>1) {
            foreach ($TableUser as $k => $val) {
                if ($val->user_id!=$user->id) {
                    $this->RummyDeal_model->ShareWallet($user->rummy_deal_table_id, $user->id, $val->user_id);
                }
            }
    ...
    public function do_share_wallet_post()
    {
        ...
        $share = $this->RummyDeal_model->GetShareWalletById($share_wallet_id);

        $this->RummyDeal_model->UpdateShareWallet($share_wallet_id, $type);

        if ($type==1) {
            $reject = false;
            $TableUser = $this->RummyDeal_model->TableUser($user[0]->rummy_deal_table_id);
            $user_count = count($TableUser)-1;
            $ShareWallet = $this->RummyDeal_model->GetShareWalletLimit($user[0]->rummy_deal_table_id, $user_count);
            // print_r($ShareWallet);
            ...
            if (!$reject) {
                ...
                // $TotalAmount = $this->RummyDeal_model->TotalAmountOnTable($user[0]->rummy_deal_table_id);
                // $AfterComissionAmount = round($TotalAmount - round($TotalAmount*$comission/100, 2), 2);
                $this->RummyDeal_model->updateTotalWinningAmtTable($TotalAmount, $user_winning_amt, $admin_winning_amt, $user[0]->rummy_deal_table_id, 0);
                $EachAmount = round($user_winning_amt/count($TableUser), 2);
                foreach ($TableUser as $ke => $val) {
                    $this->RummyDeal_model->AddToWallet($EachAmount, $val->user_id); // Add Amount to User Amount
            ...
            // $user1 = $this->RummyDeal_model->GameUserCard($game->id,$slide->user_id);
            // $user2 = $this->RummyDeal_model->GameUserCard($game->id,$slide->prev_id);
            // $remain_game_users[] = $user1;
            // $remain_game_users[] = $user2;

            // $user1 = $this->RummyDeal_model->CardValue($remain_game_users[0]->card1,$remain_game_users[0]->card2,$remain_game_users[0]->card3);
            // $user2 = $this->RummyDeal_model->CardValue($remain_game_users[1]->card1,$remain_game_users[1]->card2,$remain_game_users[1]->card3);

            // $winner = $this->RummyDeal_model->getWinnerPosition($user1,$user2);
            ...
            // $this->RummyDeal_model->PackGame($looser_id,$game->id);
        }

        // $lastChal = $this->RummyDeal_model->LastChaal($game->id);

        // $seen = $lastChal->seen;
        // $amount = $lastChal->amount;

        // $card_seen = $this->RummyDeal_model->isCardSeen($game->id,$share->user_id);
        ...
        // $this->RummyDeal_model->Chaal($game->id,$amount,$share->user_id);
    ...
    public function ask_start_game_post()
    {
        ...
        if ($this->RummyDeal_model->GetStartGame($user->rummy_deal_table_id, '0')) {
        ...
        if (count($TableUser)>1) {
            foreach ($TableUser as $k => $val) {
                if ($val->user_id!=$user->id) {
                    $this->RummyDeal_model->StartGame($user->rummy_deal_table_id, $user->id, $val->user_id);
                }
            }
    ...
    public function do_start_game_post()
    {
        ...
        $share = $this->RummyDeal_model->GetStartGameById($start_game_id);

        $this->RummyDeal_model->UpdateStartGame($start_game_id, $type);

        if ($type==1) {
            $reject = false;
            $TableUser = $this->RummyDeal_model->TableUser($user[0]->rummy_deal_table_id);
            $user_count = count($TableUser)-1;
            $StartGame = $this->RummyDeal_model->GetStartGameLimit($user[0]->rummy_deal_table_id, $user_count);
            // print_r($ShareWallet);
    ...
    public function start_game_cards($user_id)
    {
        ...
        $Cards = $this->RummyDeal_model->GetCards(count($table_data));
        // $game_data = [
        //     'table_id' => $user[0]->rummy_deal_table_id,
        ...
        foreach ($table_data as $key => $value) {
            ...
            $this->RummyDeal_model->UpdateTableUserCard($value->id, $table_user_data);
        }
    ...
    public function start_game($user_id)
    {
        ...
        $Cards = $this->RummyDeal_model->GetCards((count($table_data)*RUMMY_CARDS)+1);
        $game_data = [
            'table_id' => $user[0]->rummy_deal_table_id,
            'amount' => count($table_data)*$amount,
            'joker' => $Cards[0]->cards,
    ...
    public function user_card_post()
    {
        ...
        $game = $this->RummyDeal_model->getActiveGameOnTable($user[0]->rummy_table_id);
        ...
        foreach ($game_users as $key => $value) {
            $cards = $this->RummyDeal_model->getMyCards($game->id, $value->user_id);
    ...
    public function swap_card_post()
    {
        ...
        $game = $this->RummyDeal_model->getActiveGameOnTable($user[0]->rummy_pool_table_id);
        ...
        $isCardAvailable = $this->RummyDeal_model->getMyCards($game->id, $user_id, $my_card);

        if ($isCardAvailable) {
            $this->RummyDeal_model->SwapCards($user_id, $game->id, $my_card, $new_card);
    ...
    public function get_available_cards_post()
    {
        ...
        $data['message'] = 'Success';
        $data['code'] = HTTP_OK;
        $data['cards_list'] = $this->RummyDeal_model->GetGameCard($game->id);
        $data['joker'] = $game->joker;
        $this->response($data, HTTP_OK);
        exit();
