...
class Game extends REST_Controller
{
    ...
    public function get_table_post()
    {
        ...
        $min_wallet = $isMaster[0]->boot_value*50;
        if ($user[0]->wallet<$min_wallet) {
            $data['message'] = 'Required Minimum '.number_format($min_wallet).' Coins to Play';
            // if ($user[0]->wallet<30) {
        ...
        if (empty($TableId)) {
            ...
            if ($robot_teenpatti==0) {
                $bot = $this->Users_model->GetFreeBot($min_wallet);
    ...
    public function switch_table_post()
    {
        ...
        $seat_position = 1;
        // $tables = $this->Game_model->getPublicActiveTable();
        $tables = $this->Game_model->getCustomizeActiveTable($table_amount);
        ...
        if (empty($TableId)) {
            ...
            $robot_teenpatti = $this->Setting_model->Setting()->robot_teenpatti;
            if ($robot_teenpatti==0) {
                $min_wallet = $table->boot_value*50;
                $bot = $this->Users_model->GetFreeBot($min_wallet);
                if ($bot) {
                    $table_bot_data = [
                        'table_id' => $TableId,
    ...
    public function start_game_post()
    {
        ...
        $RobotCards = $this->Game_model->GetRobotCards(1);
        if (!empty($RobotCards)) {
            array_push($robot_card_selected, $RobotCards[0]->card1);
            array_push($robot_card_selected, $RobotCards[0]->card2);
            array_push($robot_card_selected, $RobotCards[0]->card3);
        }
