...
class RummyPool extends REST_Controller
{
    private $data;
    ...
    public function get_table_master_post()
    {
        ...
        if ($user[0]->rummy_pool_table_id) {
            $table_data = $this->RummyPool_model->TableUser($user[0]->rummy_pool_table_id);
        ...
        $table_data = $this->RummyPool_model->getTableMaster();
    ...
    public function get_table_post()
    {
        ...
        $isMaster = $this->RummyPool_model->getTableMaster($this->data['boot_value']);
        if (empty($isMaster)) {
        ...
        $tables = $this->RummyPool_model->getCustomizeActiveTable($table_amount);
        $seat_position = 1;
        ...
        if ($tables) {
            foreach ($tables as $value) {
                if ($value->members<$isMaster[0]->no_of_players) {
                    ...
                    $table_games = $this->RummyPool_model->getAllGameOnTable($value->rummy_pool_table_id);
                    if (count($table_games)==0) {
                        $TableId = $value->rummy_pool_table_id;
                        $seat_position = $this->RummyPool_model->GetSeatOnTable($TableId);
                    }
                    // }
                }
            }
        ...
        if (empty($TableId)) {
            ...
            $TableId = $this->RummyPool_model->CreateTable($table_data);
            ...
            //     $this->RummyPool_model->AddTableUser($table_bot_data);
            // }
        }
    ...
    public function get_public_table_post()
    {
        ...
        $tables = $this->RummyPool_model->getPublicActiveTable();

        foreach ($tables as $key => $value) {
            $table_games = $this->RummyPool_model->getAllGameOnTable($value->id);
    ...
    public function join_table_post()
    {
        ...
        $table = $this->RummyPool_model->isTableAvail($this->data['table_id']);
        if (!$table) {
        ...
        // $table_user = $this->RummyPool_model->TableUser($table->id);
        ...
        $table_games = $this->RummyPool_model->getAllGameOnTable($table->id);
        if (count($table_games)!=0) {
        ...
        $table_user_data = [
            'table_id' => $table->id,
            'user_id' => $user[0]->id,
            'seat_position' => $this->RummyPool_model->GetSeatOnTable($table->id),
            'added_date' => date('Y-m-d H:i:s'),
            'updated_date' => date('Y-m-d H:i:s')
        ];
        ...
        $table_data = $this->RummyPool_model->TableUser($table->id);
    ...
    public function start_game_post()
    {
        ...
        $last_game = $this->RummyPool_model->getLastGameOnTable($user[0]->rummy_pool_table_id);
        if ($last_game) {
        ...
        $table_games = $this->RummyPool_model->getAllGameOnTable($user[0]->rummy_pool_table_id);
        $amount = (count($table_games)==0) ? $table->boot_value : 0;

        $Cards = $this->RummyPool_model->GetCards((count($new_table_data)*RUMMY_CARDS)+1);
        $game_data = [
        ...
        $GameId = $this->RummyPool_model->Create($game_data);

        // $Cards = $this->RummyPool_model->GetCards(count($table_data)*13);
        ...
        foreach ($new_table_data as $key => $value) {
            ...
            if ($amount!=0) {
                $this->RummyPool_model->MinusWallet($value->user_id, $amount);
            }

            $this->RummyPool_model->AddGameCount($value->user_id);
            ...
            $this->RummyPool_model->AddGameLog($game_log);
        }
    ...
    public function rejoin_game_amount_post()
    {
        ...
        $game = $this->RummyPool_model->getLastGameOnTable($table_id);
        $rejoin_log = $this->RummyPool_model->rejoin_log($table_id, $this->data['user_id']);
    ...
    public function rejoin_game_post()
    {
        ...
        $this->RummyPool_model->MinusWallet($this->data['user_id'], $rejoin_amount);
        ...
        $this->RummyPool_model->Rejoin($rejoin_data);
    ...
    public function leave_table_post()
    {
        ...
        if ($game) {
            ...
            if (count($game_users)==1) {
                ...
                $this->RummyPool_model->updateTotalWinningAmtTable($TotalAmount, $user_winning_amt, $admin_winning_amt, $user[0]->rummy_pool_table_id, $game_users[0]->user_id);
                $this->RummyPool_model->AddToWallet($user_winning_amt, $game_users[0]->user_id);
            }
        }

        $table_users = $this->RummyPool_model->TableUser($user[0]->rummy_pool_table_id);
    ...
    public function pack_game_post()
    {
        ...
        // {
        $ChaalCount = $this->RummyPool_model->ChaalCount($game->id, $this->data['user_id']);

        $percent = $ChaalCount>0 ? CHAAL_PERCENT : NO_CHAAL_PERCENT;
        $this->RummyPool_model->PackGame($this->data['user_id'], $user[0]->rummy_pool_table_id, $game->id, $timeout, $this->input->post('json'), '', $percent);
        $game_users = $this->RummyPool_model->GameUser($game->id);
        ...
        if (count($game_users)==1) {
            ...
            $All_table_users = $this->RummyPool_model->TableUser($user[0]->rummy_pool_table_id);
            if (count($All_table_users)>=2) {
                ...
                if (count($All_table_users)==$exceed_count) {
                    ...
                    $admin_winning_amt = round($TotalAmount * round($comission/100, 2));
                    $user_winning_amt = round($TotalAmount - $admin_winning_amt, 2);
                    // $this->RummyPool_model->updateTotalWinningAmtTable($TotalAmount, $user[0]->rummy_pool_table_id);
                    // $this->RummyPool_model->AddToWallet($TotalAmount, $winner_user_id);

                    $this->RummyPool_model->updateTotalWinningAmtTable($TotalAmount, $user_winning_amt, $admin_winning_amt, $user[0]->rummy_pool_table_id, $winner_user_id);
    ...
    public function get_card_post()
    {
        ...
        // echo $chaal;
        if ($chaal==$user_id) {
            $cards = $this->RummyPool_model->getMyCards($game->id, $user_id);
            ...
            if (empty($random_card)) {
                $this->RummyPool_model->deleteDropCard($game->id);
                $random_card = $this->RummyPool_model->GetRamdomGameCard($game->id);
            }
    ...
    public function drop_card_post()
    {
        ...
        $isCardAvailable = $this->RummyPool_model->getMyCards($game->id, $user_id, $card);
        ...
        if ($isCardAvailable) {
            ...
            $this->RummyPool_model->DropGameCards($table_user_data, $this->input->post('json'));
    ...
    public function get_drop_card_post()
    {
        ...
        $drop_card = $this->RummyPool_model->GetAndDeleteGameDropCard($game->id);
    ...
    public function declare_post()
    {
        ...
        $cards = $this->RummyPool_model->getMyCards($game->id, $this->data['user_id']);
        // echo count($cards);
        if (count($cards)>RUMMY_CARDS) {
        ...
        $remain_game_users = $this->RummyPool_model->GameUser($game->id);
        ...
        $this->RummyPool_model->Declare($data);

        // $this->RummyPool_model->Show($game->id,$amount,$this->data['user_id']);

        // $amount = 800;
        // $this->RummyPool_model->MinusWallet($lose_user_id,$amount);
        // $this->RummyPool_model->MakeWinner($game->id,$amount,$user_id);
        $data['message'] = 'Success';
        // $data['winner'] = $user_id;
        $data['code'] = HTTP_OK;
        $this->response($data, HTTP_OK);
    ...
    public function declare_back_post()
    {
        ...
        $this->RummyPool_model->Declare($data_log);

        $declare_log = $this->RummyPool_model->GameLog($game->id, '', 3);
        $declare_count = count($declare_log);
        if (count($remain_game_users)<=$declare_count) {
            $amount = 0;
            $winner_id = $declare_log[$declare_count-1]->user_id;
            // $this->RummyPool_model->MakeWinner($game->id, $amount, $winner_id);
            $comission = $this->Setting_model->Setting()->admin_commission;
            ...
            $this->RummyPool_model->MakeWinner($game->id, 0, $winner_id, 0);
    ...
    public function chat_post()
    {
        ...
        if (!empty($chat)) {
            ...
            $chat_id = $this->RummyPool_model->Chat($chat_data);
            $table_users = $this->RummyPool_model->TableUser($user[0]->rummy_pool_table_id);
        ...
        $chat_list = $this->RummyPool_model->ChatList($user[0]->rummy_pool_table_id);
        $data['message'] = 'Success';
        $data['list'] = $chat_list;
        $data['code'] = HTTP_OK;
        $this->response($data, HTTP_OK);
    ...
    public function card_value_post()
    {
        ...
        $active_game = $this->RummyPool_model->getActiveGameOnTable($user[0]->rummy_pool_table_id);
        $joker = "";
        if ($active_game) {
        ...
        $card_value = $this->RummyPool_model->CardValue('', $card_1, $card_2, $card_3, $card_4, $card_5, $card_6);
        if ($card_value) {
            // echo $joker;
            // print_r($card_value);
            if ($card_value[0]==0) {
                $card_value = $this->RummyPool_model->CardValue($joker, $card_1, $card_2, $card_3, $card_4, $card_5, $card_6);
            }
    ...
    public function status_post()
    {
        ...
        if (!empty($table_id)) {
            // $data['start_game'] = $this->RummyPool_model->GetStartGame($table_id);
            $data['chat_list'] = $this->RummyPool_model->ChatList($table_id, 1);

            $table_data = $this->RummyPool_model->TableUser($table_id);
            ...
            foreach ($table_data as $t => $u) {
                ...
                $amount = $this->RummyPool_model->Invested($table_id, $u->seat_position)->amount;
            ...
            $active_game = $this->RummyPool_model->getActiveGameOnTable($table_id);
            if ($active_game) {
                // $game_id = $active_game->id;
                $data['active_game_id'] = $active_game->id;
                $data['game_status'] = 1;
            ...
            $points = $this->RummyPool_model->GetTablePoints($table_id);
            $points_arr = array();
            ...
            $data['points'] = $this->RummyPool_model->GetTablePoints($table_id);
        }
        ...
        $game = $this->RummyPool_model->View($game_id);
        if (empty($game)) {
        ...
        $game_log = $this->RummyPool_model->GameLog($game_id, 1);

        $game_users = $this->RummyPool_model->GameAllUser($game_id);
        ...
        if ($game_log[0]->action==3) {
            ...
            // $data['game_users'] = $this->RummyPool_model->GameAllUser($game_id);
        }
        // else
        // {
        $data['game_users'] = $this->RummyPool_model->GameOnlyUser($game_id);
        $data['card_count'] = $this->RummyPool_model->GetGameTableCardCount($game_id);
        // }
        $data['chaal'] = $chaal;
        $data['game_amount'] = $game->amount;
        $data['last_card'] = $this->RummyPool_model->LastGameCard($game_id);
        $data['total_table_amount'] = $this->RummyPool_model->TotalAmountOnTable($table_id);
        $data['share_wallet'] = $this->RummyPool_model->GetShareWallet($table_id);

        // $chaalCount = $this->RummyPool_model->ChaalCount($game->id,$chaal);
        // if($chaalCount>3)
        // {
        //     $this->RummyPool_model->getMyCards($game->id,$chaal);
        // }

        if (!empty($user_id)) {
            // $data['cards'] = $this->RummyPool_model->getMyCards($game->id,$user_id);
            $table_games = $this->RummyPool_model->getAllGameOnTable($table_id);
            $data['round'] = count($table_games);
            if ($active_game) {
                $data['drop_card'] = $this->RummyPool_model->GetGameDropCard($active_game->id);
            } else {
        ...
        if ($game->winner_id>0) {
            ...
            foreach ($data['game_users'] as $key => $value) {
                ...
                $game_users_cards[$key]['user']->score = (isset($win_points[$value->user_id])) ? $win_points[$value->user_id] : 0;
                $game_users_cards[$key]['user']->total = $this->RummyPool_model->getTotalPoints($table_id, $value->user_id);
                $game_users_cards[$key]['user']->cards = json_decode($this->RummyPool_model->GameLogJson($game->id, $value->user_id));
            }
    ...
    public function share_wallet_post()
    {
        ...
        $TableUser = $this->RummyPool_model->TableUser($user->rummy_pool_table_id);
        if (count($TableUser)>1) {
            foreach ($TableUser as $k => $val) {
                if ($val->user_id!=$user->id) {
                    $this->RummyPool_model->ShareWallet($user->rummy_pool_table_id, $user->id, $val->user_id);
                }
            }
    ...
    public function do_share_wallet_post()
    {
        ...
        $share = $this->RummyPool_model->GetShareWalletById($share_wallet_id);

        $this->RummyPool_model->UpdateShareWallet($share_wallet_id, $type);

        if ($type==1) {
            $reject = false;
            $TableUser = $this->RummyPool_model->TableUser($user[0]->rummy_pool_table_id);
            $user_count = count($TableUser)-1;
            $ShareWallet = $this->RummyPool_model->GetShareWalletLimit($user[0]->rummy_pool_table_id, $user_count);
            // print_r($ShareWallet);
            ...
            if (!$reject) {
                ...
                $this->RummyPool_model->updateTotalWinningAmtTable($user_winning_amt, $user_winning_amt, $admin_winning_amt, $user[0]->rummy_pool_table_id, 0);
                $EachAmount = round($user_winning_amt/count($TableUser), 2);
                foreach ($TableUser as $ke => $val) {
                    $this->RummyPool_model->AddToWallet($EachAmount, $val->user_id); // Add Amount to User Amount
            ...
            // $user1 = $this->RummyPool_model->GameUserCard($game->id,$slide->user_id);
            // $user2 = $this->RummyPool_model->GameUserCard($game->id,$slide->prev_id);
            // $remain_game_users[] = $user1;
            // $remain_game_users[] = $user2;

            // $user1 = $this->RummyPool_model->CardValue($remain_game_users[0]->card1,$remain_game_users[0]->card2,$remain_game_users[0]->card3);
            // $user2 = $this->RummyPool_model->CardValue($remain_game_users[1]->card1,$remain_game_users[1]->card2,$remain_game_users[1]->card3);

            // $winner = $this->RummyPool_model->getWinnerPosition($user1,$user2);
            ...
            // $this->RummyPool_model->PackGame($looser_id,$game->id);
        }

        // $lastChal = $this->RummyPool_model->LastChaal($game->id);

        // $seen = $lastChal->seen;
        // $amount = $lastChal->amount;

        // $card_seen = $this->RummyPool_model->isCardSeen($game->id,$share->user_id);
        ...
        // $this->RummyPool_model->Chaal($game->id,$amount,$share->user_id);
    ...
    public function ask_start_game_post()
    {
        ...
        if ($this->RummyPool_model->GetStartGame($user->rummy_pool_table_id, '0')) {
        ...
        if (count($TableUser)>1) {
            foreach ($TableUser as $k => $val) {
                if ($val->user_id!=$user->id) {
                    $this->RummyPool_model->StartGame($user->rummy_pool_table_id, $user->id, $val->user_id);
                }
            }
    ...
    public function do_start_game_post()
    {
        ...
        $share = $this->RummyPool_model->GetStartGameById($start_game_id);

        $this->RummyPool_model->UpdateStartGame($start_game_id, $type);

        if ($type==1) {
            $reject = false;
            $TableUser = $this->RummyPool_model->TableUser($user[0]->rummy_pool_table_id);
            $user_count = count($TableUser)-1;
            $StartGame = $this->RummyPool_model->GetStartGameLimit($user[0]->rummy_pool_table_id, $user_count);
            // print_r($ShareWallet);
    ...
    public function start_game_cards($user_id)
    {
        ...
        $Cards = $this->RummyPool_model->GetCards(count($table_data));
        // $game_data = [
        //     'table_id' => $user[0]->rummy_pool_table_id,
        ...
        foreach ($table_data as $key => $value) {
            ...
            $this->RummyPool_model->UpdateTableUserCard($value->id, $table_user_data);
        }
    ...
    public function start_game($user_id)
    {
        ...
        $Cards = $this->RummyPool_model->GetCards((count($table_data)*RUMMY_CARDS)+1);
        $game_data = [
            'table_id' => $user[0]->rummy_pool_table_id,
            'amount' => count($table_data)*$amount,
            'joker' => $Cards[0]->cards,
        ...
        $this->RummyPool_model->StartDropGameCards($table_user_data);
    ...
    public function user_card_post()
    {
        ...
        $game = $this->RummyPool_model->getActiveGameOnTable($user[0]->rummy_table_id);
        ...
        foreach ($game_users as $key => $value) {
            $cards = $this->RummyPool_model->getMyCards($game->id, $value->user_id);
    ...
    public function swap_card_post()
    {
        ...
        $isCardAvailable = $this->RummyPool_model->getMyCards($game->id, $user_id, $my_card);

        if ($isCardAvailable) {
            $this->RummyPool_model->SwapCards($user_id, $game->id, $my_card, $new_card);
    ...
    public function get_available_cards_post()
    {
        ...
        $data['message'] = 'Success';
        $data['code'] = HTTP_OK;
        $data['cards_list'] = $this->RummyPool_model->GetGameCard($game->id);
        $data['joker'] = $game->joker;
        $this->response($data, HTTP_OK);
        exit();
